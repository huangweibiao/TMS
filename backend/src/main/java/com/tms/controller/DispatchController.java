...
public class DispatchController {
     ...
     */
    @PostMapping("/assign")
    public Result<DispatchDTO> assignDispatch(
     ...
     */
    @PostMapping
    public Result<DispatchDTO> createDispatch(@Valid @RequestBody DispatchDTO dispatchDTO) {
     ...
     */
    @GetMapping("/{id}")
    public Result<DispatchDTO> getDispatchById(@PathVariable Long id) {
    ...
    public Result<DispatchDTO> getDispatchByWaybillId(@PathVariable Long waybillId) {
     ...
     */
    @GetMapping
    public Result<PageResult<DispatchDTO>> getDispatchList(
            @RequestParam(required = false) String dispatchNo,
            @RequestParam(required = false) Integer status,
            @RequestParam(defaultValue = "1") int pageNum,
            @RequestParam(defaultValue = "10") int pageSize) {
        PageResult<DispatchDTO> result = dispatchService.getDispatchList(dispatchNo, status, pageNum, pageSize);
        return Result.success(result);
    }
