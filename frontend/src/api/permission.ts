...
export const permissionApi = {
  // 获取权限列表
  getList(): Promise<Result<Permission[]>> {
  ...
  // 根据权限编码查询
  getByCode(permissionCode: string): Promise<Result<Permission>> {
    return request.get(`/api/permission/by-code/${permissionCode}`)
  },

  // 根据父权限ID查询权限列表
  getByParentId(parentId: number): Promise<Result<Permission[]>> {
    return request.get(`/api/permission/by-parent/${parentId}`)
  },

  // 根据权限类型查询权限列表
  getByType(permissionType: number): Promise<Result<Permission[]>> {
    return request.get(`/api/permission/by-type/${permissionType}`)
  },

  // 创建权限
  create(data: Partial<Permission>): Promise<Result<Permission>> {
    return request.post('/api/permission', data)
  },

  // 更新权限
  update(id: number, data: Partial<Permission>): Promise<Result<Permission>> {
  ...
  // 启用权限
  enable(id: number): Promise<Result<Permission>> {
    return request.post(`/api/permission/${id}/enable`)
  },

  // 禁用权限
  disable(id: number): Promise<Result<Permission>> {
