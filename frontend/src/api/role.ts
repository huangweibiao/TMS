...
export const roleApi = {
  ...
  // 根据角色编码查询角色
  getByCode(roleCode: string): Promise<Result<Role>> {
    return request.get(`/api/role/by-code/${roleCode}`)
  },

  // 根据状态查询角色列表
  getByStatus(status: number): Promise<Result<Role[]>> {
    return request.get(`/api/role/by-status/${status}`)
  },

  // 创建角色
  create(data: Partial<Role>): Promise<Result<Role>> {
    return request.post('/api/role', data)
  },

  // 更新角色
  update(id: number, data: Partial<Role>): Promise<Result<Role>> {
  ...
  // 启用角色
  enable(id: number): Promise<Result<Role>> {
    return request.post(`/api/role/${id}/enable`)
  },

  // 禁用角色
  disable(id: number): Promise<Result<Role>> {
  ...
  // 获取用户角色
  getByUserId(userId: number): Promise<Result<Role[]>> {
